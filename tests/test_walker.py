...
def random_partition_graph() -> nx.Graph:
    return nx.random_partition_graph([1000] * 15, 0.01, 0.001)
...
def test_random_walks(random_partition_graph: nx.Graph):
    ...
    walks = walker.random_walks(
        random_partition_graph, n_walks=n_walks, walk_len=walk_len
    )
    assert_walk_shape(walks, n_walks, walk_len, len(random_partition_graph.nodes))


def test_generate_random_walks_with_restart_probability(
    random_partition_graph: nx.Graph,
):
    n_walks = 50
    walk_len = 25
    alpha = 0.1
    ...
    walks = walker.random_walks(
        random_partition_graph, n_walks=n_walks, walk_len=walk_len, alpha=alpha
    )
    assert_walk_shape(walks, n_walks, walk_len, len(random_partition_graph.nodes))
...
def test_p_and_q(random_partition_graph: nx.Graph):
    ...
    walks = walker.random_walks(
        random_partition_graph, n_walks=n_walks, walk_len=walk_len, p=p, q=q
    )
    assert_walk_shape(walks, n_walks, walk_len, len(random_partition_graph.nodes))
...
def test_corrupt_walks(random_partition_graph: nx.Graph):
    ...
    assert_walk_shape(
        corrupted_walks, n_walks, walk_len - 1, len(random_partition_graph.nodes)
    )
